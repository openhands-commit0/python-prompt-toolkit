...
def _split_multiline_prompt(get_prompt_text: _StyleAndTextTuplesCallable) -> tuple[Callable[[], bool], _StyleAndTextTuplesCallable, _StyleAndTextTuplesCallable]:
    """
...
class _RPrompt(Window):
    ...
    def __init__(self, text: AnyFormattedText) -> None:
        super().__init__(FormattedTextControl(text=text), align=WindowAlign.RIGHT, style='class:rprompt')
...
PromptContinuationText = Union[str, 'MagicFormattedText', StyleAndTextTuples, Callable[[int, int, int], AnyFormattedText]]
_T = TypeVar('_T')
...
class PromptSession(Generic[_T]):
    ...
    def _get_line_prefix(self, line_number: int, wrap_count: int, get_prompt_text_2: _StyleAndTextTuplesCallable) -> StyleAndTextTuples:
        """
