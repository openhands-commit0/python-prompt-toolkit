    ...
    def __init__(self, body: AnyContainer, menu_items: list[MenuItem], floats: list[Float] | None=None, key_bindings: KeyBindingsBase | None=None) -> None:
        ...
        def _click(event: E) -> None:
            ...
            if item.handler:
                event.app.layout.focus_last()
                item.handler()
        self.control = FormattedTextControl(self._get_menu_fragments, key_bindings=kb, focusable=True, show_cursor=False)
