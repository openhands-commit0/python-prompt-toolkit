...
class Dialog:
    ...
    def __init__(self, body: AnyContainer, title: AnyFormattedText='', buttons: Sequence[Button] | None=None, modal: bool=True, width: AnyDimension=None, with_background: bool=False) -> None:
        ...
        if buttons:
            frame_body = HSplit([Box(body=DynamicContainer(lambda: self.body), padding=D(preferred=1, max=1), padding_bottom=0), Box(body=VSplit(buttons, padding=1, key_bindings=buttons_kb), height=D(min=1, max=3, preferred=3))])
        else:
            frame_body = body
        kb = KeyBindings()
        kb.add('tab', filter=~has_completions)(focus_next)
        kb.add('s-tab', filter=~has_completions)(focus_previous)
        frame = Shadow(body=Frame(title=lambda: self.title, body=frame_body, style='class:dialog.body', width=None if with_background is None else width, key_bindings=kb, modal=modal))
